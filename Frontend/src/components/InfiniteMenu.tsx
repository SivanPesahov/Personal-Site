...
void main() {
  if (vInstanceId != uActiveInstance) { discard; }
...
class InfiniteGridMenu {
  ...
  private discLocations!: {
    ...
    uActiveInstance: WebGLUniformLocation | null;
  };
  ...
  private atlasSize = 1;
  private currentVisibleInstance = 0;
  private _time = 0;
  ...
  private init(onInit?: InitCallback): void {
    ...
    this.discLocations = {
      ...
      uActiveInstance: gl.getUniformLocation(
        this.discProgram!,
        "uActiveInstance"
      ),
  ...
  private render(): void {
    ...
    gl.uniform1i(
      this.discLocations.uActiveInstance,
      this.currentVisibleInstance
    );
  ...
  private onControlUpdate(deltaTime: number): void {
    ...
    if (!this.control.isPointerDown) {
      ...
      this.currentVisibleInstance = nearestVertexIndex;
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  const handleButtonClick = () => {
    ...
    try {
      ...
      // Fallback – if URL parsing fails, try client navigation
      if (link.startsWith("/")) {
        navigate(link);
      } else {
